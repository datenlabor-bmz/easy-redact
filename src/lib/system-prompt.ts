...
export function buildSystemPrompt(opts: {
  ...
  return [
    ...
    '## Mehrere Dokumente',
    '',
    'Wenn mehrere Dokumente hochgeladen sind, gibt `read_documents` ein Array von Dokumenten zurück, jedes mit `documentKey`, `documentName` und `pages`. Jedes Dokument hat seinen eigenen, bei 0 startenden Seitenindex. Verwende immer den `documentKey` aus der `read_documents`-Antwort in deinen Vorschlägen, damit die Schwärzungen dem richtigen Dokument zugeordnet werden.',
    '',
    ...
    '- `documentKey`: aus read_documents (erforderlich bei mehreren Dokumenten)',
    '- `text`: Exakter Text aus dem Dokument',
    '- `pageIndex`: Seitennummer (0-basiert, innerhalb des jeweiligen Dokuments)',
    ...
    '- `documentKey`: aus read_documents',
    ...
    '- `documentKey`: aus read_documents',
    '- `fromPage`/`toPage`: Erste und letzte Seite (0-basiert, inklusiv, innerhalb des Dokuments)',
    '- `confidence`, `person`, `personGroup`, `reason`',
    '',
