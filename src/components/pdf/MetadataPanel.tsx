    ...
    <div className='w-full max-w-[var(--page-width,700px)] mx-auto my-3 px-2'>
      <div className='rounded-lg border border-border bg-card overflow-hidden'>
        <div className='px-3 py-2 border-b border-border'>
          <span className='text-xs font-semibold text-muted-foreground'>Metadaten</span>
        </div>
          ...
          {keys.map(k => {
            ...
            return (
              <div key={k} className='flex items-center gap-3 px-3 py-1.5'>
                <span className='text-[11px] text-muted-foreground w-24 shrink-0'>{FIELD_LABELS[k] ?? k}</span>
                ...
                <button onClick={() => toggle(k)}
                  ...
                  <span className='text-[10px] text-muted-foreground group-hover:text-foreground transition-colors'>Entfernen</span>
